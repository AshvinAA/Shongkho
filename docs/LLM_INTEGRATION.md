...
| 4 | Window contents | append the whole `{message, ui_blocks}` envelope | Append `message` + tool **names/args** only. Payloads never re-enter the window. | Re-sending 60-bucket payloads every turn contradicts v2's own "re-fetch, don't remember" principle and is pure token waste. |
